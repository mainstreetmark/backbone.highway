	...
	/*
		...
		describe( '#_updateToFirebase', function () {
			...
			it( 'should call update on a Firebase reference', function () {
				...
				collection = new Collection();
				collection._updateToFirebase( collection.firebase, {
					id: '1'
				} );
				expect( collection.firebase.update.calledOnce )
					.to.be.ok;
			} );
			...
			describe( 'calling Backbone.Collection.prototype._prepareModel', function () {
				...
				beforeEach( function () {
					User = Backbone.Model.extend( {} ),
						Users = window.Backbone.Highway.Collection.extend( {
							url: 'Mock://',
							idAttribute: User.prototype.idAttribute,
							initialize: function ( models, options ) {
								this.model = function ( attrs, opts ) {
									return new User( _.extend( attrs, {
				...
				it( 'should call Backbone.Collection.prototype._prepareModel', function () {
					...
					expect( Backbone.Collection.prototype._prepareModel.calledOnce )
						.to.be.ok;
					Backbone.Collection.prototype._prepareModel.restore();
				} );

				it( 'should prepare models', function () {
					var addedArray = collection.add( {
						firstname: 'Dave'
					} );
					var addedObject = addedArray[ 0 ];
					expect( addedObject.addedFromCollection )
						.to.be.ok;
				} );
		...
		describe( 'SyncCollection', function () {
			...
			it( 'should call add when added', function () {
				var spy = sinon.spy();
				var Models = window.Backbone.Highway.Collection.extend( {
					url: 'Mock://',
					autoSync: true
				} );
			...
			describe( '#remove', function () {
				...
				// call _setWithCheck
				it( 'should call window.Backbone.Highway._setWithCheck', function () {
					...
					expect( window.Backbone.Highway._setWithCheck.calledOnce )
						.to.be.ok;

					window.Backbone.Highway._setWithCheck.restore();
				} );
			...
			describe( '#fetch', function () {
				...
				it( 'should call window.Backbone.Highway._promiseEvent', function () {
					sinon.spy( Backbone.Firebase, '_promiseEvent' );

					collection.fetch();
					collection.firebase.flush();

					expect( window.Backbone.Highway._promiseEvent.calledOnce )
						.to.be.ok;

					window.Backbone.Highway._promiseEvent.restore();
				} );
			...
			describe( '#_updateModel', function () {
				...
				it( 'should call _setWithPriority if the .priority property is present', function () {
					...
					collection._updateModel( model );
					expect( collection._setWithPriority.calledOnce )
						.to.be.ok;
				...
				it( 'should call _updateToFirebase if no .priority property is present', function () {
					...
					collection._updateModel( model );
					expect( collection._updateToFirebase.calledOnce )
						.to.be.ok;
		...
		describe( 'OnceCollection', function () {
			...
			describe( '#fetch', function () {
				...
				it( 'should call window.Backbone.Highway.sync', function () {
					sinon.spy( Backbone.Firebase, 'sync' );

					collection.fetch();

					expect( window.Backbone.Highway.sync.calledOnce )
						.to.be.ok;

					window.Backbone.Highway.sync.restore();
				} );
