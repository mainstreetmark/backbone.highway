	...
	var HighwayCollection = ( function () {
		...
		HighwayCollection.protoype = {
			...
			filter: function ( iteratee, context ) {
				if ( this.minimum !== {} ) {
					// Should I fetch the whole collection and do a filter? It seems like you almost need to.
					var self = this;
					return new Promise( function ( success, failure ) {
							self.io.emit( 'search', {}, function ( records ) {
								success( records );
							} );
						} )
						.then( function ( records ) {
							Backbone.Collection.prototype.add.call( self, records );
							return Backbone.Collection.prototype.filter.call( self, iteratee, context );
						} );
				} else
					return Backbone.Collection.prototype.filter.call( this, iteratee, context );
			},
			where: function ( attrs, first ) {
				if ( this.minimum !== {} ) {
					// Where is easy enough.  Search for a bunch on the server and append them to the collection
					this.io.emit( 'search', attrs, function ( records ) {

					} );
				} else
					return Backbone.Collection.prototype.where.call( this, attrs, first );
			},
			findWhere: function ( attrs ) {
				// I don't think I even need to overload this, because it just falls into .where()
				return Backbone.Collection.prototype.findWhere.call( this, attrs );
			},
			any: function ( iteratee, context ) {
				// need to fetch all and perform.
				return Backbone.Collection.prototype.any.call( this, iteratee, context );
			},
			contains: function ( iteratee, context ) {
				// need to fetch all and perform
				return Backbone.Collection.prototype.contains.call( this, iteratee, context );
			},
