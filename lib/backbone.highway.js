(function (Backbone, _, io) {
			...
			set: function (key, val, options) {
				...
				if (this.collection && this.collection.io && this.get('_id') && !options.dontSave && !options.silent) {
					...
					if (this.collection.io.connected === false) {
						var collection = this.collection.url.split('/').pop();
						var d = localStorage.getItem('backfill_' + collction) || '[]';
						d = JSON.parse(d);
						d.push(data);
						localStorage.setItem('backfill_' + collction, JSON.stringify(d));
					} else {
	...
	var HighwayCollection = (function () {
		...
		HighwayCollection.protoype = {
			...
			_backfillData: function () {
				var collectionName = this.url.split('/').pop();
				var data = localStorage.getItem('backfill_' + collectionName);
				console.log('Backfilling data for ' + collectionName, data);
				if (data) {
					console.log('null is valid, I guess')
