	...
	const HighwayCollection = (function () {
		...
		HighwayCollection.protoype = {
			...
			_create(model, options) {
				...
				console.log('_create model', model.getName());
			...
			_childChanged(snap) {
				...
				console.log('child changed', item.getName());
			...
			_updateModel(model, options) {
				if (!model || model instanceof HighwayModel)
					return;
				console.log('child updated', model.getName());
			...
			_addModel(model) {
				console.log('HW add model', model.getName());
				const self = this;
				if (!model || model.attributes._id || this._remoteChanging) {
					console.log('highway:model_created');
