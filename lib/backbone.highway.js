(function bh(Backbone, _, io) {
	...
	const POLLER_NAME = 'TG'; // "TG" is a magic word that says to use localStorage for backfilling
	if (argv) {
		const path = process.argv[1].split('/');
		const POLLER_NAME = path[path.length - 2];
	}
		...
		HighwayModel.prototype = {
			...
			clear(options) {
				...
				for (const key in this.attributes)
					if (key !== '_id') attrs[key] = undefined;
