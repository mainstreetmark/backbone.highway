		...
		HighwayCollection.protoype = {
			...
			add: function (models, options) {
				if (!_.isArray(models))
					models = [models];
				var out;
				out = Backbone.Collection.prototype.add.call(this, models, options);
				for (var i in models) {
					console.log(models[i]);
					models[i].trigger('change');
				}
				return out;
			},
			...
			_updateModel: function (model) {
				...
				if (!model._remoteChanging && model.id) {
					...
					console.log('child_added', model.toJSON());
					this.io.emit('child_added', model.toJSON());
				}
			},
