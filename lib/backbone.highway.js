( function ( Backbone, _ ) {
	...
	var HighwayCollection = ( function () {
		...
		HighwayCollection.protoype = {
			...
			create: function ( model, options ) {
				var self = this;
				options = options ? _.clone( options ) : {success: function(){}};
				if ( !model ) {
					return false;
				}
				var set = self.add( [ model ], options );
				this.io.emit( 'create', model, function ( data ) {
					set[0].set(data, { silent: true, trigger: false });
	...
	var Model = Backbone.Model.extend( {
		set: function ( key, val, options ) {
			...
			for ( k in data ) {
				if ( k.indexOf( '.' ) > -1 ) {
					obj = JSON.parse(JSON.stringify(this.get( k.split( '.' ).shift() )));
					this._setRecursive( obj, k.split( '.' ).splice( 1 ).join( '.' ), data[ k ] );
					data[ k.split( '.' ).shift() ] = obj;
					delete data[ k ];
				}
			}
		...
		get: function ( key ) {
			...
			return (typeof val == 'object' && val.length) ? _.clone(val) : val;
		}
	} );
