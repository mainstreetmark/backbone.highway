(function (Backbone, _, io) {
	...
	var HighwayCollection = (function () {
		...
		function HighwayCollection() {
			...
			_.defer(_.bind(function () {
				this.io.emit('init', this.constraints.minimum);
			}, this));
		...
		HighwayCollection.protoype = {
			...
			add: function (models, options) {
				...
				this._addModel(set);
				if (!_.isEmpty(this.constraints.maximum)) {
					var limit = this.constraints.maximum.limit || this.length;
			...
			_search: function (options) {
				var self = this;
				if (!_.isEmpty(this.constraints.minimum) && this._partial === true) {
					return new Promise(function (success, failure) {
			...
			_where: function (attr, first) {
				var self = this;
				if (!attr)
					return Promise.reject('No attr supplied');
				return new Promise(function (success, failure) {
					self._search({
							search: attr
						})
			...
			_childChanged: function (snap) {
				...
				var findFunc = function (child) {
					return child.get('_id') == model._id || !child.id;
				};

				var item = _.find(this.models, findFunc);
	...
	var Collection = Backbone.Collection.extend({
		constructor: function (model, options) {
			...
			this._partial = !_.isEmpty(this.constraints.minimum) || !_.isEmpty(this.constraints.maximum);
