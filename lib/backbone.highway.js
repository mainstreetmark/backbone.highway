	...
	var HighwayModel = (function () {
		...
		HighwayModel.prototype = {
			...
			set: function (key, val, options) {
				...
				if(this.collection && this.collection.io && this.get('_id') && !options.dontSave){
					console.log('Setting via io', options);
					this.collection.io.emit('update', $.extend(true, data, { "_id" : this.get('_id')}) );
				}
		...
		HighwayCollection.protoype = {
			...
			remove: function (models, options) {
				...
				for (var i in models) {
					...
					case 'boolean':
					case 'undefined':
						console.error('Called Backbone.Highway.Collection.remove with invalid model: ', model);
						return;
					}
