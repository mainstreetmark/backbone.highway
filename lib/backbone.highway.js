(function bh(Backbone, _, io) {
	...
	let POLLER_NAME = 'TG'; // "TG" is a magic word that says to use localStorage for backfilling
	if (argv) {
		const path = process.argv[1].split('/');
		POLLER_NAME = path[path.length - 2];
	}
	const HighwayModel = (function hm() {
				...
				if (this.collection && this.collection.io && this.get('_id') && !options.dontSave && !options.silent) {
					...
					if (this.collection.io.connected === false) {
						...
						if (POLLER_NAME === 'TG') {
							...
							case 'events':
							case 'events_archive':
								console.warn(`${this.getName()}: Offline.  Sending ${collection} to backfill log`);
								fs.appendFileSync(`/tmp/tg_${POLLER_NAME}_${collection}.txt`, `${JSON.stringify(data)}\n`);
							}
						}
					} else {
				...
				if (key && key.indexOf('.') > -1) {
					...
					if (val)
						for (let i = 1; i < pieces_length; i++)
							val = typeof val === 'object' ? val[pieces[i]] : undefined;
				} else {
	...
	const HighwayCollection = (function hc() {
		...
		HighwayCollection.protoype = {
			...
			_addModel(model) {
				...
				if (this.io.connected) {
					...
					const fs = require('fs');
					const collection = this.url.split('/').pop();
					console.log('Backlogging new model', model.attributes);
					fs.appendFileSync(`/tmp/tg_${POLLER_NAME}_${collection}.txt`, `${JSON.stringify(model.attributes)}\n`);
				}
			},
			...
			_backfillData() {
				...
				if (POLLER_NAME === 'TG') {
					let data = localStorage.getItem(`backfill_${collection}`);
					if (data !== null) {
						...
						localStorage.setItem(`backfill_${collection}`, '[]');
					}
				} else {
					const fs = require('fs');
					fs.readFile(`/tmp/tg_${POLLER_NAME}_${collection}.txt`, 'utf8', (err, data) => {
	 				if (!err && data) {
							const lines = data.split('\n');
							console.log(`backfilling ${collection}, ${lines.length} data points`);
							for (const l in lines) {
								let json;
								if (lines[l].length > 2) {
									json = JSON.parse(lines[l]);
									this.io.emit('update', json);
								}
							}
							fs.unlinkSync(`/tmp/tg_${POLLER_NAME}_${collection}.txt`);
						}
					});
				}
	...
	const Collection = Backbone.Collection.extend({
		constructor(model, options) {
			...
			this.io.on('disconnect', function () { console.warn('OFFLINE', this.nsp); });
			// this.io.on('connect', function() { console.warn("ONLINE");});
			...
			this.io.on('connect_timeout', console.warn.bind(this, 'Connection Timeout'));
